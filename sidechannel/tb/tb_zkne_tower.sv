// ============================================================================
//  tb_zkne_tower.sv - functional check of the tower-field cv32e40p_zkne
// ----------------------------------------------------------------------------
//  Drives all 256 input bytes through aes32esi (operand_a = 0, bs = 0) and
//  confirms result[7:0] == standard AES S-box. Catches any subtle SV semantics
//  bug in the inlined-function form before we spend 18 min on a bitstream.
// ============================================================================
`timescale 1ns/1ps

module tb_zkne_tower
  import cv32e40p_pkg::*;
();
  ...
  initial begin
    ...
    for (int i = 0; i < 256; i++) begin
      ...
      if (result[7:0] !== expected[i]) begin
        errors = errors + 1;
        if (errors <= 4) begin
          $display("MISMATCH x=0x%02x: result=0x%08x expected[7:0]=0x%02x",
                   i[7:0], result, expected[i]);
        end
      end
    end

    if (errors == 0)
      $display("TOWER ZKNE: PASS  (all 256 ESI byte-0 outputs match)");
    else
      $display("TOWER ZKNE: FAIL  errors=%0d/256", errors);
    $finish;
  end
