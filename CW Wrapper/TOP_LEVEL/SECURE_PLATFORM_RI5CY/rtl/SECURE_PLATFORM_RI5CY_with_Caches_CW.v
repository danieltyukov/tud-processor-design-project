...
/////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
module SECURE_PLATFORM_RI5CY_CW
		#(
		...
		(
                ...
                input        [127:0]                   crypto_pt,		
                output reg   [127:0]                  crypto_ct,
                input                                 crypto_start,
                output reg                            crypto_done,
                output                               crypto_trigger
				//output		[31:0]						output_LEDS,
				//output		[31:0]						output_LEDS2,
				//input       [1:0]                       SWITCHES,
...
assign          SERIAL_CTS = 0;   //UART doesn't drive CTS pin
assign          SERIAL_RST = 1;   //UART doesn't drive RST pin
...
// AHB_CACHE #(.MEM_ADDR_BITS(10),.INSTRUCTION(1)) uAHB2MEM (
//	//AHBLITE Signals
//	.HSEL(1'b1),
//	.HCLK(sys_clock), 
//	.HRESETn(sys_reset_N), 
...
//);
 ...
 AHB2BRAM_DP #(.MEMDEPTH(12)) uAHB2MEM_DP (

	.HCLK(sys_clock), 
	.HRESETn(sys_reset_N), 
	
	//AHBLITE Signals for inst
	.ins_HSEL(1'b1),
	.ins_HREADY(ins_HREADY),     
	.ins_HADDR(HADDR_INST[31:0]),
	.ins_HTRANS(ins_HTRANS), 
	.ins_HWRITE(ins_HWRITE),
	.ins_HSIZE(ins_HSIZE),
	.ins_HWDATA(ins_HWDATA), 
	...
	.ins_HRDATA(ins_HRDATA), 
	.ins_HREADYOUT(ins_HREADY),	
	
	
	//AHBLITE Signals for data
	.dat_HSEL(HSEL_MEM),
	.dat_HREADY(dat_HREADY),     
	.dat_HADDR(HADDR_MEM),
	.dat_HTRANS(dat_HTRANS), 
	...
	.dat_HRDATA(HRDATA_MEM), 
	.dat_HREADYOUT(HREADYOUT_MEM)	
);
...
////////AHBLite Data Memory
////assign HADDR_MEM = {16'd0,dat_HADDR[15:0]};
//AHB_CACHE #(.MEM_ADDR_BITS(10),.INSTRUCTION(0)) uAHB2DMEM (
//	//AHBLITE Signals
//	.HSEL(HSEL_MEM),
//	.HCLK(sys_clock), 
//	.HRESETn(sys_reset_N), 
...
reg              crypto_start_tmp;
reg  [127:0]     crypto_key_i;
reg  [127:0]     crypto_pt_i;
wire [127:0]     crypto_ct_i;
wire             crypto_done_tmp;
...
AHB2CW uAHB2CW (
	...
	.crypto_key(crypto_key_i),
    .crypto_pt(crypto_pt_i),
    .crypto_ct(crypto_ct_i),
    .crypto_start(crypto_start_tmp),
    .crypto_done(crypto_done_tmp),
...
////AHBLite AES Accelerator
//assign HADDR_AES = {16'd0,dat_HADDR[15:0]};
//AHB2AES uAHB2AES (
//	//AHBLITE Signals
//	.HSEL(HSEL_AES),
//	.HCLK(sys_clock), 
...
//);	
assign HREADYOUT_AES = 1'b1;
// AHBLite UART Pheripheral - to PC TERM
assign HADDR_UART = 0;//{16'd0,dat_HADDR[15:0]};
/*AHB2UART uAHBUART(
	.HSEL(HSEL_UART),
	.HCLK(sys_clock),
	.HRESETn(sys_reset_N),
...
// AHBLite timer
assign HADDR_TIMER = 0;//{16'd0,dat_HADDR[15:0]};
/*
    AHB2TIMER uAHBTIMER(
    .HSEL(HSEL_TIMER),
	.HCLK(sys_clock),
...
assign HADDR_NI = 0;//{16'd0,dat_HADDR[15:0]};
/*
(* dont_touch = "true" *) AHB2NI #(
	.FIFO_SIZE(10'd32),
	.MY_ADDRESS(MY_ADDRESS),
...
*/
reg trigger_pulse;
always @(posedge clock, negedge sys_reset_N)  
if (!sys_reset_N)
    trigger_pulse <= 1'b0;
else if (crypto_done_tmp == 1'b1)
    trigger_pulse <= 1'b1;
else if (crypto_done_tmp == 1'b0) 
    trigger_pulse <= 1'b0;

wire trigger_1;
assign trigger_1 = trigger_pulse & !crypto_done_tmp;
...
always @(posedge clock, negedge sys_reset_N)  
if (!sys_reset_N)
       crypto_start_tmp <= 1'b0;
else if(crypto_start == 1'b1)
	   crypto_start_tmp <= 1'b1;
else if(crypto_done_tmp == 1'b1)
	   crypto_start_tmp <= 1'b0;
