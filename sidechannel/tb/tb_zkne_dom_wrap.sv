// ============================================================================
//  tb_zkne_dom_wrap.sv - functional check of the DOM-masked cv32e40p_zkne
// ----------------------------------------------------------------------------
//  Drives all 256 input bytes through aes32esi (operand_a = 0, bs = 0), each
//  via a 5-cycle multi-cycle handshake (IDLE -> P1..P4 -> DONE), and confirms
//  the unmasked output byte == standard AES S-box. Verifies ready_o handshake
//  and back-to-back AES instructions don't corrupt the pipeline.
// ============================================================================
`timescale 1ns/1ps

module tb_zkne_dom_wrap
  import cv32e40p_pkg::*;
();
  ...
  // expected S-box (FIPS 197 Appendix B)
  ...
  initial begin
    ...
    // sequentially: for each x in 0..255, drive aes32esi, wait for ready, sample
    for (int i = 0; i < 256; i++) begin
      // present the AES operation
      @(negedge clk);
      ...
      // wait until ready_o asserts (~5 cycles expected, cap at 20 for safety)
      begin
        int n;
        n = 0;
        while (!ready && n < 20) begin
          @(posedge clk);
          n = n + 1;
        end
        if (!ready) begin
          $error("ready_o never asserted for x=0x%02x", i);
          $finish;
        end
      end

      // sample result at the cycle ready_o is high
      if (result[7:0] !== expected[i]) begin
        errors++;
        if (errors <= 4) begin
          $display("MISMATCH x=0x%02x: result=0x%02x  expected=0x%02x",
                   i[7:0], result[7:0], expected[i]);
        end
      end

      // step past this instruction: pretend the ALU advanced
      @(negedge clk);
      operator = ALU_ADD;     // non-AES op
      @(posedge clk);          // allow FSM to return to IDLE
    end

    if (errors == 0)
      $display("DOM WRAP ZKNE: PASS  (all 256 ESI byte-0 outputs match)");
    else
      $display("DOM WRAP ZKNE: FAIL  errors=%0d/256", errors);
    $finish;
  end

  initial begin
    #200_000;
    $error("TIMEOUT in tb_zkne_dom_wrap");
    $finish;
  end
